...
// Example: const REMOTE_API_BASE_URL = 'https://api.mahalakshmihomepickles.com';
...
const RAZORPAY_CONFIG_URL = API_BASE_URL ? new URL('/api/config', API_BASE_URL).toString() : '/api/config';
let razorpayPublicKey = null;

async function loadRazorpayKeyId() {
  if (razorpayPublicKey) return razorpayPublicKey;

  try {
    const response = await fetch(RAZORPAY_CONFIG_URL);
    if (!response.ok) {
    ...
    razorpayPublicKey = data.keyId;
    return razorpayPublicKey;
  } catch (error) {
    console.error(error);
    showNotification('Unable to load payment gateway configuration. Make sure your backend is running and accessible from this website.', 'error');
    throw error;
  }
}
...
class CartDrawer {
  ...
  checkout() {
    ...
    showOrderModal(this.cart, total, 'Please review your cart and confirm your payment details.', orderNumber);
  }
...
if (orderForm) {
  orderForm.addEventListener('submit', (event) => {
    ...
    showNotification('For secure online payment, select products from the catalogue and use the cart or checkout flow.', 'info');
  });
}
...
async function payNow(orderData) {
  const packagingFee = 30;
  const deliveryCharge = 50;
  const amountINR = Number(orderData.total) + packagingFee + deliveryCharge;

  if (!Number.isFinite(amountINR) || amountINR <= 0) {
    console.error('Invalid payment amount:', orderData.total, packagingFee, deliveryCharge);
    showNotification('Unable to create payment order: invalid order amount.', 'error');
    return;
  }
  ...
  let razorpayKey;
  try {
    razorpayKey = await loadRazorpayKeyId();
  } catch (error) {
    return;
  }

  if (typeof window.Razorpay === 'undefined') {
    showNotification('Razorpay checkout library is not loaded.', 'error');
    return;
  }

  let paymentOrder;
  try {
    const createResponse = await fetch(`${API_BASE_URL}/api/create-order`, {
      method: 'POST',
    ...
    if (!createResponse.ok) {
      const errorData = await createResponse.json().catch(async () => {
        try {
          return { error: await createResponse.text() };
        } catch {
          return {};
        }
      });
      const message = errorData.error || errorData.message || `Failed to create payment order (status ${createResponse.status}).`;
      throw new Error(message);
    }

    paymentOrder = await createResponse.json();
  } catch (error) {
    console.error('Create order failed:', error);
    showNotification(`Unable to create payment order: ${error.message}`, 'error');
    return;
  }
      ...
      try {
        const verifyResponse = await fetch(`${API_BASE_URL}/api/verify-payment`, {
          ...
          body: JSON.stringify({
            ...
            amount: orderData.total + packagingFee + deliveryCharge,
        ...
        showNotification('Payment could not be verified. Please contact support.', 'error');
      }
    },
    prefill: {
      name: orderData.name || '',
...
function showOrderModal(items, total, message, orderNumber) {
  ...
  const emptyMessageHtml = !hasItems ? '<div class="order-message"><p>Please select products from the catalogue and use the checkout flow to pay online.</p></div>' : '';
  ...
  modal.innerHTML = `
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button class="btn-primary modal-confirm">${hasItems ? 'Pay Now' : 'Close'}</button>
      </div>
    </div>
  `;
  ...
  modal.innerHTML = `
    ...
    <div class="modal-content">
      ...
      <div class="modal-body success-body">
        ...
        <p>Your payment of <strong>₹${data.amountPaid}/-</strong> has been received.</p>
        ...
        <p>We have sent confirmation to your email and WhatsApp (if enabled).</p>
      </div>
      <div class="modal-footer">
